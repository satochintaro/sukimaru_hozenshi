...
function fmtDate(v){if(!v)return"–";const d=new Date(v);if(Number.isNaN(d.getTime()))return esc(v);return d.toLocaleString("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});}
function svgRadar(data){const w=360,h=280,cx=180,cy=134,r=88,levels=5;let out=`<svg viewBox="0 0 ${w} ${h}" aria-label="科目別レーダーチャート">`;for(let l=1;l<=levels;l++){const rr=r*l/levels,pts=data.map((_,i)=>{const a=-Math.PI/2+Math.PI*2*i/data.length;return`${(cx+Math.cos(a)*rr).toFixed(1)},${(cy+Math.sin(a)*rr).toFixed(1)}`});out+=`<polygon points="${pts.join(" ")}" fill="none" stroke="#d8dee5"/>`;}data.forEach((d,i)=>{const a=-Math.PI/2+Math.PI*2*i/data.length,x=cx+Math.cos(a)*r,y=cy+Math.sin(a)*r,lx=cx+Math.cos(a)*(r+38),ly=cy+Math.sin(a)*(r+28);out+=`<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="#d8dee5"/><text x="${lx}" y="${ly}" text-anchor="middle" font-size="11" fill="#68737e">${esc(d.short)}</text><text x="${lx}" y="${ly+13}" text-anchor="middle" font-size="11" font-weight="700" fill="#20262d">${d.value}%</text>`;});const pts=data.map((d,i)=>{const a=-Math.PI/2+Math.PI*2*i/data.length,rr=r*d.value/100;return`${(cx+Math.cos(a)*rr).toFixed(1)},${(cy+Math.sin(a)*rr).toFixed(1)}`});out+=`<polygon points="${pts.join(" ")}" fill="rgba(40,120,184,.18)" stroke="#2878B8" stroke-width="2"/></svg>`;return out;}
function svgBars(data){const w=520,h=220,ml=116,mr=42,mt=10,barH=22,gap=14,gw=w-ml-mr;let out=`<svg viewBox="0 0 ${w} ${h}" aria-label="科目別平均正答率">`;data.forEach((d,i)=>{const y=mt+i*(barH+gap),bw=gw*d.value/100;out+=`<text x="8" y="${y+15}" font-size="12" fill="#68737e">${esc(d.label)}</text><rect x="${ml}" y="${y}" width="${gw}" height="${barH}" rx="6" fill="#e5e9ed"/><rect x="${ml}" y="${y}" width="${bw}" height="${barH}" rx="6" fill="${d.color}"/><text x="${ml+gw+8}" y="${y+15}" font-size="12" font-weight="800" fill="#20262d">${d.value}%</text>`;});return out+"</svg>";}
function analyze(){const accs=rows.map(r=>r.total?r.correct/r.total*100:0),answers=rows.map(r=>r.total),cats={};SUBJ.forEach(c=>cats[c]={c:0,t:0});const weak={};rows.forEach(r=>{for(const c in r.cats){if(!cats[c])cats[c]={c:0,t:0};cats[c].c+=Number(r.cats[c].c)||0;cats[c].t+=Number(r.cats[c].t)||0;}(r.alerts||[]).forEach(a=>{const k=a.id??a.text;if(!weak[k])weak[k]={id:a.id,text:a.text,cat:a.cat,n:0,names:new Set()};weak[k].names.add(r.name);weak[k].n=weak[k].names.size;});});const shorts=["生産基本","日常保全","効率・ロス","改善・解析","保全基礎"];const catData=SUBJ.map((c,i)=>({label:c,short:shorts[i],value:pct(cats[c].c,cats[c].t)||0,color:CC[c],sample:cats[c].t}));return{avg:rows.length?Math.round(accs.reduce((a,b)=>a+b,0)/rows.length):0,med:Math.round(median(accs)),sd:Math.round(stddev(accs)),range:accs.length?Math.round(Math.max(...accs)-Math.min(...accs)):0,avgAnswers:rows.length?Math.round(answers.reduce((a,b)=>a+b,0)/rows.length):0,today:rows.filter(r=>new Date(r.receivedAt||r.sentAt).toDateString()===new Date().toDateString()).length,mastered:rows.length?Math.round(rows.reduce((a,r)=>a+r.mastered,0)/rows.length):0,avgAlerts:rows.length?Math.round(rows.reduce((a,r)=>a+r.alerts.length,0)/rows.length*10)/10:0,catData,weak:Object.values(weak).sort((a,b)=>b.n-a.n).slice(0,10)};}
...
function individualAdvice(r){const total=r.total||0,acc=total?Math.round(r.correct/total*100):0;let weakCat=null,weakRate=101;SUBJ.forEach(c=>{const x=r.cats[c];if(x&&x.t){const v=Math.round(x.c/x.t*100);if(v<weakRate){weakRate=v;weakCat=c;}}});if(total<30)return{level:"データ不足",cls:"info",text:`まず30問以上を回答して傾向を確認。現在は${total}問のため、教育判断は仮置き。`};if(acc<60)return{level:"個別基礎フォロー",cls:"danger",text:`${weakCat||"基礎項目"}を中心に、指導者同席で10問ずつ確認。正解だけでなく理由を口頭説明する。`};if(acc<75)return{level:"重点補講",cls:"warn",text:`最弱科目「${weakCat}」を15分補講し、誤答問題を翌日と3日後に再演習。`};if((r.alerts||[]).length>=5)return{level:"定着フォロー",cls:"warn",text:`正答率は${acc}%だが要注意が${r.alerts.length}問。1日・3日・7日の間隔で復習し、まぐれ正解を減らす。`};if(acc>=90)return{level:"応用・教える役",cls:"good",text:`理解度が高い。現場事例問題や、他者へ説明する役割で理解をさらに定着させる。`};return{level:"弱点演習",cls:"info",text:`「${weakCat}」の代表問題5問を理由付きで解き、1週間後に再確認する。`};}
function educationHtml(a){const valid=a.catData.filter(x=>x.sample>0),weakest=[...valid].sort((x,y)=>x.value-y.value)[0],second=[...valid].sort((x,y)=>x.value-y.value)[1];if(!weakest)return"";const lev=levelFor(weakest.value),content=EDUCATION[weakest.label]||{title:"弱点科目の補強",practice:"代表問題と現場事例を組み合わせて補講する。"};const recs=[];
  if(a.avgAnswers<30)recs.push({pri:"A",cls:"warn",title:"まず診断データを増やす",body:`1人平均${a.avgAnswers}問です。最低30問、できれば各科目10問以上を回答してから本格的な教育判断を行う。`});
  recs.push({pri:a.avgAnswers<30?"B":"A",cls:lev.cls,title:`最優先科目：${weakest.label}（${weakest.value}%）`,body:`${lev.label}。${content.title}：${content.practice} 実施手順は「${lev.plan}」。`});
  if(second&&second.value<75)recs.push({pri:"B",cls:"warn",title:`第2優先：${second.label}（${second.value}%）`,body:`最優先科目の補講後、代表問題5問と現場事例1件で短時間フォローする。`});
  if(a.sd>=12||a.range>=25)recs.push({pri:"A",cls:"info",title:"一斉教育ではなく2グループ化",body:`正答率のばらつきは標準偏差${a.sd}pt、最大差${a.range}pt。70%未満は基礎組、85%以上は応用組に分けると教育効率が上がる。`});
