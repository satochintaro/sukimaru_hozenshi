...
(() => {
  ...
  function updateVersionDisplay(){
    document.title=document.title.replace(/5\.\d+(?:\.\d+)?/g,"5.7.0");
    if(current==="academic"){
      const status=document.querySelector("#sc-set .sts span:last-child");
      if(status)status.innerHTML='Ver 5.7.0 / QUEST UI ／ 全 <span id="st-qn">1000</span> 問';
    }else{
      const heroInfo=document.querySelector(".pt-hero small");
      if(heroInfo)heroInfo.textContent="問題バンク60課題・毎回ランダム10課題";
      const sub=document.querySelector("#pt-home .hd-sub");
      if(sub)sub.textContent="PLAYER / QUEST UI / 実技";
    }
  }

  async function loadEnhancements(){
    try{
      loadStyle("./theme-v57.css");
      loadStyle("./coach.css");
      await loadScript("./ui-v57.js");
      ...
      if(current==="academic"){
        ...
        if(typeof APP!=="undefined"&&APP)APP.version="5.7.0-quest";
      }else{
      ...
      console.error("Ver 5.7.0 QUEST UI 拡張読込エラー",error);
    }
  }
